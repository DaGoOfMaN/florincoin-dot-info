                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                    var newTx = new Tx({
                      ...
                      txcomment: tx['tx-comment'],
                      txcomment_exists: ( (tx['tx-comment'] != null) && (tx['tx-comment'] != '') && (tx['tx-comment'] != "") ),
                    });
...
module.exports = {
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({'txcomment_exists': true}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        //console.log(txs);
